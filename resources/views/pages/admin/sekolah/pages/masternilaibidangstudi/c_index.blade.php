            ...
            <table id="example" class="table table-striped table-bordered mt-1" style="width:100%">
                ...
                @forelse ($datas as $data)
                        <tr id="sid{{ $data->id }}">
                                ...
                                <td>
                                    {{$data->singkatan}}
                                    {{-- {{$data->walimasternilaibidangstudi->nama}} --}}
                                </td>

                                <td class="text-center">
                                    {{-- <x-button-reset-pass link="/admin/{{ $pages }}/{{$data->id}}/reset" /> --}}
                                    <x-button-edit link="{{ route('sekolah.masternilaibidangstudi.edit',[$id->id,$data->id])}}" />
                                    <x-button-delete link="{{ route('sekolah.masternilaibidangstudi.edit',[$id->id,$data->id])}}" />
                                </td>
                            </tr>
                @empty
                            <tr>
