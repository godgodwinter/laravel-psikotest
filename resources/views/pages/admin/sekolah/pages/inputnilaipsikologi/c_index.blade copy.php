            ...
            <table id="example" class="table table-striped table-bordered mt-1" style="width:100%">
                ...
                <tbody>
                @forelse ($dataakhir_array as $data)
                        <tr id="sid{{ $data['id'] }}">
                                <td class="text-center">
                                    {{$data['id']}}
                                <td> {{Str::limit($data['nama'],25,' ...')}}
                                </td>
                                @foreach ($data['masternilai'] as $master)
                                    <td class="text-center">
                                        <input type="text" class="form-control form-control-plaintext text-center" value="{{$master['nilai']}}" readonly id="inputan_{{$data['id']}}">
                                        {{-- {{dd($master['nilai'])}} --}}
                                    </td>
                                    <script>
