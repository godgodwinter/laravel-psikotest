...
use App\Exports\exportBuletin;
use App\Exports\exportdetailsekolah;
use App\Exports\exportKatabijak;
use App\Exports\exportKlasifikasiJabatan;
use App\Exports\exportPenanganan;
use App\Exports\exportsekolah;
use App\Exports\exportTerapis;
...
class prosescontroller extends Controller
{
    ...
    public function exportKlasifikasiJabatan()
    {
        $tgl = date("YmdHis");
        return Excel::download(new exportKlasifikasiJabatan, 'klasifikasijabatan-' . $tgl . '.xlsx');
    }

    public function exportBuletin()
    {
        $tgl = date("YmdHis");
        return Excel::download(new exportBuletin, 'buletin-infomasipsikolgi-' . $tgl . '.xlsx');
    }


    public function exportTerapis()
    {
        $tgl = date("YmdHis");
        return Excel::download(new exportTerapis, 'terapis-penjelasan-' . $tgl . '.xlsx');
    }
    ...
    public function exportPenanganan()
    {
        ...
        // dd($datas);
        $tgl = date("YmdHis");
        return Excel::download(new exportPenanganan, 'penanganan-pemecahan-' . $tgl . '.xlsx');
    }
    public function exportKatabijak()
    {

        // dd($datas);
        $tgl = date("YmdHis");
        return Excel::download(new exportKatabijak, 'katabijak-' . $tgl . '.xlsx');
    }
    ...
    public function backuptemp(Request $request)
    {
        ...
        foreach ($datas as $data) {
            ...
            $client = new \GuzzleHttp\Client();
            $responsedeteksi = $client->request(
                'POST',
